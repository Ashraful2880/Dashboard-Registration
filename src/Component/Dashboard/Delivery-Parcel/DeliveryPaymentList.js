...
const DeliveryPaymentList = () => {
    ...
    const pageCount = Math.ceil(deliveryPaymentLists?.length / dataPerPage);
    ...
    useEffect(() => {
        fetch("/pickUpParcelList.json")
            .then(res => res.json())
            .then(data => setDeliveryPaymentLists(data))
    }, [])
        ...
        <div className="px-4 mx-auto">
            ...
            <div className="border border-gray-200 pb-5 rounded-md shadow-xl">
                ...
                <div className="lg:flex block justify-between items-center border-b py-3 px-10">
                    <h4 className="text-md font-bold">Delivery Payment List</h4>
                    <div className="lg:my-0 my-2">
                        <Link to="/dashboard/deliveryPaymentGenerate" className="bg-green-700 hover:bg-green-800 duration-300 py-2 px-4 rounded-md text-white text-md font-semibold mx-2">
                            <FontAwesomeIcon icon={faPen} className="text-white mr-1" /> Generate Delivery Payment
                        </Link>
                    </div>
                </div>
                        ...
                        <div className="shadow overflow-hidden rounded-lg">
                            <table className="min-w-full divide-y divide-x divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-x divide-gray-200 text-gray-900 text-center text-sm font-normal">
                                    {deliveryPaymentLists.slice(pagesVisited, pagesVisited + dataPerPage).map((parcel) => (
                ...
                <div className="lg:flex block justify-between items-center my-2 mx-10">
                    <div className="border border-green-700 px-2 py-1 rounded-md hidden lg:block">
                        <p>Showing <span className="font-semibold">{pagesVisited + 1}</span> to <span className="font-semibold">{deliveryPaymentLists.slice(0, pagesVisited + dataPerPage).length}</span> of <span className="font-semibold">{deliveryPaymentLists?.length}</span> Entries</p>
                    </div>

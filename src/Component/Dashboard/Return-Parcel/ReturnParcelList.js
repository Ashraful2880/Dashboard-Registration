...
import { faPen, faPrint, faEye, faPenToSquare } from '@fortawesome/free-solid-svg-icons';
...
const ReturnParcelList = () => {
    let ref = useRef();
    const [returnParcelLists, setReturnParcelLists] = useState([]);
    const [printData, setPrintData] = useState();
    // Pagination Function Here
    const [showData, setShowData] = useState(0);
    const [dataPerPage, setDataPerPage] = useState(10);
    const pagesVisited = showData * dataPerPage;
    const pageCount = Math.ceil(returnParcelLists?.length / dataPerPage);
    ...
    useEffect(() => {
        fetch("/pickUpParcelList.json")
            .then(res => res.json())
            .then(data => setReturnParcelLists(data))
    }, [])

    return (
        <div className="px-4 mx-auto">
            <h3 className="text-2xl font-bold mb-6 text-left">Return Parcel List</h3>
            <div className="border border-gray-200 py-5 rounded-md shadow-xl">
                ...
                <div className="flex justify-between items-center mx-10 mb-5">
                    <h3 className="font-bold text-lg lg:block hidden">Return Parcels List</h3>
                    <div>
                        <Link to="/dashboard/generateReturnRiderRun" className="bg-green-800 border border-green-800 text-white py-2 px-4 rounded-md hover:bg-transparent hover:text-green-800 duration-300 font-semibold">
                        ...
                        <div className="shadow overflow-hidden rounded-lg">
                            <table className="min-w-full divide-y divide-x divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-x divide-gray-200 text-gray-900 text-center text-sm font-normal">
                                    {returnParcelLists.slice(pagesVisited, pagesVisited + dataPerPage).map((deliveryParcel) => (
                ...
                <div className="lg:flex block justify-between items-center my-2 mx-10">
                    <div className="border border-green-700 px-2 py-1 rounded-md">
                        <p>Showing <span className="font-semibold">{pagesVisited + 1}</span> to <span className="font-semibold">{returnParcelLists.slice(0, pagesVisited + dataPerPage).length}</span> of <span className="font-semibold">{returnParcelLists?.length}</span> Entries</p>
                    </div>

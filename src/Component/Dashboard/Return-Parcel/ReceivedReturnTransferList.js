...
const ReceivedReturnTransferList = () => {
    ...
    const pageCount = Math.ceil(receivedReturnTransList?.length / dataPerPage);
            ...
            <div className="border border-gray-200 py-5 rounded-md shadow-xl">
                        ...
                        <div className="shadow overflow-hidden rounded-lg">
                            <table className="min-w-full divide-y divide-x divide-gray-200">
                                ...
                                <tbody className="bg-white divide-y divide-x divide-gray-200 text-gray-900 text-center text-sm font-normal">
                                    {receivedReturnTransList.slice(pagesVisited, pagesVisited + dataPerPage).map((deliveryParcel) => (
                ...
                <div className="lg:flex block justify-between items-center my-2 mx-10">
                    <div className="border border-green-700 px-2 py-1 rounded-md hidden lg:block">
                        <p>Showing <span className="font-semibold">{pagesVisited + 1}</span> to <span className="font-semibold">{receivedReturnTransList.slice(0, pagesVisited + dataPerPage).length}</span> of <span className="font-semibold">{receivedReturnTransList?.length}</span> Entries</p>
                    </div>

...
const GenerateReturnRiderRun = () => {
    ...
    useEffect(() => {
        fetch("/pickUpParcelList.json")
            .then(res => res.json())
            .then(data => setGenRiderRun(data))
    }, [])

    return (
        <div className="px-4 mx-auto">
            <h3 className="text-2xl font-bold mb-6 text-left text-gray-500">Generate Return Rider Run</h3>
            <div className="lg:flex block justify-between w-full">
                <form className="border border-gray-200 py-5 rounded-md shadow-md lg:w-1/2 w-full lg:px-10 px-6 lg:mx-6 mx-0">
                    <h2 className="text-green-700 text-xl font-bold text-left lg:mb-5 mb-2">Return Run</h2>
                    ...
                    <div className="w-full flex">
                        <input className="text-lg border rounded-md mt-4 border-green-700 mx-2 py-1.5 px-5 bg-green-700 text-white hover:text-green-800 hover:bg-transparent duration-300 w-1/2" type="submit" value="Generate" />
                        <input className="text-lg border rounded-md mt-4 border-red-600 mx-2 py-1.5 px-5 bg-red-600 text-white hover:text-red-600 hover:bg-transparent duration-300 w-1/2" type="reset" value="Reset" />
                    </div>
                </form>
...
export default GenerateReturnRiderRun;